...
if(!class_exists('mysql_utilites')):
	...
	//db instance
		...
		//moeld manufactuarere
		function model_manufacturer($man='',$model=''){
			if($man == '' && $model == '') return;
			
			$id = 'wp_' . preg_replace('/[ ]/','',$man);
			$mans_model = get_option($id);
			$mans_model[] = trim($model);
			$new_model = array_unique($mans_model);
			update_option($id,$new_model);
